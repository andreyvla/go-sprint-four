...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь
	speedKmInH := meanSpeed(action, duration)
	speedMInS := speedKmInH * kmhInMsec
	speedInSquare := math.Pow(speedMInS, 2)
	return (walkingCaloriesWeightMultiplier*weight + (speedInSquare/height/cmInMgi)*walkingSpeedHeightMultiplier*weight) * duration * minInH
}
